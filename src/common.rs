...
pub fn is_public(_url: &str) -> bool {
    // Fork override (self-hosted only): never treat any rendezvous server as the
    // public rustdesk.com cloud. Keep this on upstream merges — upstream checks
    // for "rustdesk.com" here, which must not apply to the hospital deployment.
    false
}
